...
namespace SendPulse
{
	public class SendPulseClient : ISendPulseClient
	{
		...
		public SendPulseClient(string userId, string secret)
		{
			_userId = userId ?? throw new ArgumentNullException("userId");
			_secret = secret ?? throw new ArgumentNullException("secret");

			this.tokenName = md5(_userId + "::" + _secret);
